...
infileName <- file.path(baseDir, "report", "allPreMetrics.csv")
outfilePrefix <- file.path(baseDir, "report", "allPreMetrics")
rm(list=ls())
...
myData <- read.csv(infileName, stringsAsFactors = FALSE, header = TRUE, row.names=1)
myData <- myData[rownames(sampleTab),]
metrics <- grep("averageLength.*BeforeFilter", colnames(myData), value = TRUE, invert = TRUE)
myData <- myData[,metrics]
myData$time <- sampleTab$time
myData$lab <- sampleTab$lab
myData$batchWithin <- sampleTab$batch

#########################################################################################
# test for differences
matrixCols <- paste0(c("percSS", "pValue"), '_', rep(c("time", "lab", "batchWithin"), each = 2))
allTests <- matrix(NA, ncol = length(matrixCols), nrow = length(metrics), dimnames = list(metrics, matrixCols))
for (curMetric in metrics) {
  res <- anova(lm(terms(as.formula(paste0(curMetric, '~ time + batchWithin + lab')), keep.order = TRUE), data = myData))
