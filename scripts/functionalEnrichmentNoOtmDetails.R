...
outfilePrefix <- file.path(baseDir, "GitIgnore_results", "enrichment", "union.5.BP.bothTimePoints")
rm(list=ls())
...
for (curSet in names(allPeaks)) {
  ...
  for (curPeaks in names(tempBedList)) {
    ...
    for (annotationType in c("homer", "chipSeeker")) {
      ...
      # GO term enrichment
      if (file.exists(file.path(rDir, paste0(enrichPrefix, "_BP_minNodeSize_5.txt")))) {
      ...
      if (file.exists(file.path(rDir, paste0(enrichPrefix, "_MF_minNodeSize_5.txt")))) {
      ...
      if (file.exists(file.path(rDir, paste0(enrichPrefix, "_CC_minNodeSize_5.txt")))) {
